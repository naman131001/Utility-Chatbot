...
def refine_query(
    ...
    """
    # Build a compact history summary (last 4 turns, roles only)
    history_text = ""
    if history:
        recent = history[-4:]
        history_text = "\n".join(
...
def build_context(hits: list[dict]) -> str:
    # Sort so NY ESCO Doc chunks appear first in context
...
#     st.markdown("---")
    ...
    top_k = st.slider("Results to retrieve", 3, 10, 15)
    ...
    enable_refinement = st.toggle(
        ...
        help=(
            ...
            "Helps with acronyms, pronouns, and vague phrasing."
        ),
    )
...
for msg in st.session_state.messages:
    with st.chat_message(msg["role"], avatar="🧑" if msg["role"] == "user" else "⚡"):
        ...
        # Show refined query used for this turn (assistant messages only)
...
if user_input:
    ...
    with st.chat_message("assistant", avatar="⚡"):
        ...
        # Build plain history (excludes current user message)
        plain_history = [
        ...
        refined_query = user_input  # default: pass-through
        if enable_refinement and search_client:
            ...
            # Show the refined query if it differs from original
        ...
        # ── Step 2: Hybrid search with refined query ───────────────────────
        with st.spinner("Searching documents…"):
            try:
                hits = hybrid_search(
                    query=refined_query,          # ← refined query used here
        ...
        # ── Step 3: Generate answer with original question + retrieved chunks
        if not hits:
            answer = (
            ...
            with st.spinner("Generating answer…"):
                try:
                    answer = generate_answer(
                        question=user_input,      # ← original question preserved
                        hits=hits,
                        history=plain_history,
                        openai_client=openai_client,
                        chat_deploy=chat_deploy,
